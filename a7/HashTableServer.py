...
class HashTableServer:
    def __init__(self, proj, host=socket.gethostname(), port=0, peer_id=None, base_project_name=None):
        self.host = host
        self.port = port
        self.peer_id = peer_id
        self.project_name = proj
 
        # base_project_name is passed explicitly from Peer.py where it is
        # already known cleanly, avoiding fragile string splitting on hyphens.
        self.base_project_name = base_project_name or proj
    ...
    def execute(self, req, caller_host=None):
        k = req.get("key")
        v = req.get("value")
