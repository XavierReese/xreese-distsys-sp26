...
class HashTable:
    def __init__(self, peer_id=None):
        ...
        suffix = f"-{self.peer_id}" if self.peer_id else ""
        self.ckpt_fn = f"table{suffix}.ckpt"
        self.log_fn = f"table{suffix}.txn"
    ...
    def _log(self, m, k=None, v=None, details=None):
        try:
            with open(self.log_fn, "a", encoding='utf-8') as f:
    ...
    def _startup(self):
        ...
        try:
            nfiles = 0
            with open(self.ckpt_fn, "r", encoding='utf-8') as f:
                for line in f:
        ...
        try:
            nfiles = 0
            with open(self.log_fn, "r", encoding='utf-8') as f:
                for line in f:
                    data = line.strip().split("::")
                    if data[1] == "insert":
